...
fun EditNoteScreen(
    ...
    LaunchedEffect(true) {
        viewModel.writeNoteToState(noteId)
    }
    LaunchedEffect(true) {
        viewModel.event.collect { event ->
            when(event) {
                EditNoteScreenEvent.OnSuccess -> {onCloseClick()}
                is EditNoteScreenEvent.ShowError -> {
    ...
    Box(modifier = Modifier.fillMaxSize().background(MaterialTheme.colorScheme.background)) {
        ...
        Column(
            ...
            Row(
                ...
                Button(
                    onClick = { action(EditNoteScreenAction.OnSaveClick(noteId)) },
            ...
            BasicTextField(
                value = state.title,
                onValueChange = { action(EditNoteScreenAction.OnTitleChange(it)) },
                textStyle = TextStyle(
            ...
            BasicTextField(
                ...
                onValueChange = { action(EditNoteScreenAction.OnRelatedToChange(it)) },
                textStyle = TextStyle(
                    color = MaterialTheme.colorScheme.onSurface,
                    fontSize = 18.sp,
                    fontWeight = FontWeight.ExtraBold
            ...
            BasicTextField(
                value = state.body,
                onValueChange = { action(EditNoteScreenAction.OnBodyChange(it)) },
                textStyle = TextStyle(
                    color = MaterialTheme.colorScheme.onSurface,
                    fontSize = 18.sp,
                    lineHeight = 26.sp
